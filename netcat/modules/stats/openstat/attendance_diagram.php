...
foreach ($report['item'] as $item) {
    $items[] = array($item['c'][0],
            $item['c'][1],
            $item['c'][2],
            strtotime($item['v']));
    if ($item['c'][0] < $y_min) $y_min = $item['c'][0];
    if ($item['c'][0] > $y_max) $y_max = $item['c'][0];
    if ($item['c'][1] < $y_min) $y_min = $item['c'][1];
    if ($item['c'][1] > $y_max) $y_max = $item['c'][1];
    if ($item['c'][2] < $y_min) $y_min = $item['c'][2];
    if ($item['c'][2] > $y_max) $y_max = $item['c'][2];
}
unset($report['item']);
...
drawLegend($img, $report, $caption_color, $color1, $color2, $color3, NETCAT_MODULE_STATS_OPENSTAT_VISITORS,
        NETCAT_MODULE_STATS_OPENSTAT_SESSIONS, NETCAT_MODULE_STATS_OPENSTAT_PAGEVIEWS);
...
function drawLegend($img, $report, $caption_color, $color1, $color2, $color3, $caption1, $caption2, $caption3) {
    ...
    if (!$nc_core->NC_UNICODE) $caption1 = $nc_core->utf8->win2utf($caption1);
    ...
    if (!$nc_core->NC_UNICODE) $caption2 = $nc_core->utf8->win2utf($caption2);
    ...
    if (!$nc_core->NC_UNICODE) $caption3 = $nc_core->utf8->win2utf($caption3);
