...
class nc_subscriber_admin {
    ...
    public function settings_save() {
        // валидация данных
        $settings = array();
        $settings['MergeMail'] = intval($this->POST['MergeMail']);
        $settings['FromName'] = $this->db->escape($this->POST['FromName']);
        $settings['FromEmail'] = $this->db->escape($this->POST['FromEmail']);
        $settings['ReplyEmail'] = $this->db->escape($this->POST['ReplyEmail']);
        $settings['TestEmail'] = $this->db->escape($this->POST['TestEmail']);
        $settings['Charset'] = $this->db->escape($this->POST['Charset']);
        $settings['EmailField'] = $this->db->escape($this->POST['EmailField']);
        $settings['ConfirmType'] = intval($this->POST['ConfirmType']);
        $settings['ConfirmSubject'] = $this->db->escape($this->POST['ConfirmSubject']);
        $settings['ConfirmBody'] = $this->db->escape($this->POST['ConfirmBody']);
        $settings['ConfirmHTML'] = intval($this->POST['ConfirmHTML']);
        $settings['TextConfirm'] = $this->db->escape($this->POST['TextConfirm']);
        $settings['TextUnscribe'] = $this->db->escape($this->POST['TextUnscribe']);
        $settings['TextError'] = $this->db->escape($this->POST['TextError']);
        $settings['MaxMailCount'] = intval($this->POST['MaxMailCount']);
        $settings['MaxTime'] = intval($this->POST['MaxTime']);
        $settings['FormSubscribe'] = $this->db->escape($this->POST['FormSubscribe']);
