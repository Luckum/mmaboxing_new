...
{nc_ctpl:f(a,"edit_template_id"),isModal:1});j(a,!0).each(function(a){b["item_ids["+a+"]"]=d(this).data("itemId")});parent.nc_form(NETCAT_PATH+"index.php?",null,null,null,"GET",b)}).on("click",".nc-netshop-variant-delete-all",function(a){a.preventDefault();var a=g(a.target),a=d.extend(f(a,"request_parameters"),l(a),{"delete":1,posting:0}),b="",c;for(c in a)b+='<input type="hidden" name="'+c.toString().replace('"',"&quot;")+'" value="'+a[c].toString().replace('"',"&quot;")+'" />\n';d("<form />",{action:NETCAT_PATH+
"message.php?",method:"POST",html:b}).appendTo("body").submit()}).on("click",".nc-netshop-variant-add-multiple",function(a){a.preventDefault();a=g(a.target);a=NETCAT_PATH+"modules/netshop/admin/item/create_bulk_dialog.php?component_id="+f(a,"component_id")+"&parent_item_id="+f(a,"parent_item_id");nc.ui.modal_dialog({url:a,on_show:p}).open()});h()});
