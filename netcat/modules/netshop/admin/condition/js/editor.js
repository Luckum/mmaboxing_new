 ...
 * @param {Object} data
 * @param {Number} siteId
 * @param {Array} conditionGroupsToShow
 * @param {Array} conditionGroupsToExclude
 */
function nc_netshop_condition_editor(container, input_name, data, siteId, conditionGroupsToShow, conditionGroupsToExclude) {
    this.loadStylesIn(window);

    this.siteId = siteId;
    this.root = $nc(container).addClass('nc-netshop-condition-editor');
    this.inputField = $nc("<input type='hidden' name='" + input_name + "'/>")
                          .appendTo(this.root);

    if (data && !$nc.isEmptyObject(data)) { this.inputField.val(JSON.stringify(data)); }
    this.conditionGroupsToShow = conditionGroupsToShow || [];
    this.conditionGroupsToExclude = conditionGroupsToExclude || [];
...
(function($) {
    ...
    var Class = nc_netshop_condition_editor,
        ...
        adminFilesPath = NETCAT_PATH + 'admin/',
        filesPath = NETCAT_PATH + 'modules/netshop/admin/condition/',
        dataPath = filesPath + 'data/'; /* ??? */
    ...
    Class.urls = {
        DynamicTreeScript: NETCAT_PATH + 'admin/js/tree_frame.js',
        SelectComponentList: dataPath + "json/component_list.php",
        ObjectName: dataPath + "json/object_name.php",
        ObjectSubdivisionTree: dataPath + "json/subdivision_tree.php?site_id=%siteId%&node=",
        ObjectList: dataPath + "html/object_list.php",
        ...
        UserGroupTree: dataPath + "json/user_group_tree.php?node=",
        UserList: dataPath + "html/user_list.php",
    ...
    // Dialog ids (oh boy, they’re long)
    Class.dialogIds = {
        itemSelection: 'nc_netshop_condition_editor_item_selection_dialog',
        userSelection: 'nc_netshop_condition_editor_user_selection_dialog'
    };
    ...
    // operator sets
    var opEqNe = { name: 'op', type: 'SimpleSelect', values: {
            eq: lang.EQUALS,
            ne: lang.NOT_EQUALS
        }}, // равно-не равно
        opContains = { name: 'op', type: 'SimpleSelect', values: {
            contains: lang.CONTAINS,
            notcontains: lang.NOT_CONTAINS
        }},
        opString = { name: 'op', type: 'SimpleSelect', values: {
            eq: lang.EQUALS,
            ne: lang.NOT_EQUALS,
            contains: lang.CONTAINS,
            notcontains: lang.NOT_CONTAINS,
        ...
        opNumber = { name: 'op', type: 'SimpleSelect', values: {
        ...
        opDateTime = opNumber,
        opAll = $.extend({}, opString);

    $.extend(opAll.values, opNumber.values);

    var makeSubdivisionField = function(fieldName) {
            return {
    ...
    var makeComponentField = function(fieldName) {
            return {
                name: fieldName || 'value',
                type: 'SelectFromJson',
    ...
    Instance.conditionTypes = {
        ...
        datetime_timeinterval: {
            ...
            params: [
                lang.TIME_FROM,
                { name: 'time_from', type: 'Input', inputType: 'time', 'class': 'nc--small' },
                lang.TIME_TO,
                { name: 'time_to', type: 'Input', inputType: 'time', 'class': 'nc--small' }
            ]
        },
    ...
    Instance.showAddConditionDialog = function(target) {
        var options = [
            ...
            groupsToSkip = this.conditionGroupsToExclude;
        ...
        for (var typeName in this.conditionTypes) {
            ...
            if (groupsToSkip.length && $.inArray(conditionData.group, groupsToSkip) != -1) {
                continue;
            }
    ...
    Instance.updateOperatorLabelPosition = function(groupContainer) {
        ...
        if (conditions.length > 1) {
            ...
            var wasVisible = operatorLabel.hasClass('visible'),
                ...
                top = firstTop + 20 + 0.5 * (lastTop - firstTop - operatorLabel.height());
    ...
    Instance.addCondition = function(conditionType, container, values) {
        ...
        $("<a class='remove-condition-button'></a>")
...
//    };
     ...
     *
     */
    Instance.createDialog = function() {
        var $ = window.top.$nc,
            html = '<div class="nc-netshop-condition-dialog">' +
                        ...
                        '</div>' +
                        '<div class="nc_admin_form_body"></div>' +
                        '<div class="nc_admin_form_buttons"></div>' +
                   '</div>',
            dialog = $(html).hide();

        this.loadStylesIn(window.top);
        $('body').append(dialog);

        // handle window size changes
        dialog.data("onResize", function(dialogContainer) {
            $(".column", dialogContainer).height(dialogContainer.height());
        });
     ...
     *
     */
    Instance.createTreeDialog = function(dialogOptions) {
        var $ = window.top.$nc,
            dialogId = dialogOptions.dialogId,
            treeId = dialogId + "_tree",

            dialog = this.createDialog().attr('id', dialogId),

            body = dialog.find(".nc_admin_form_body"),
            buttonContainer = dialog.find(".nc_admin_form_buttons");

        // --- dialog contents ---
        // header
        dialog.find(".nc_admin_form_menu h2").html(dialogOptions.headerText);

        // body
        /* "menu_left_block" class is required for the dynamicTree :( */
        body.append('<div class="column column1 tree-column menu_left_block">' +
                        '<ul class="tree" id="' + treeId + '" />' +
                    '</div>' +
                    '<div class="column column2 list-column" />');

        // buttons
        buttonContainer.append(
//            $("<button />", {
//                'class': "nc_admin_metro_button nc-btn nc--blue",
//                html: lang.DIALOG_SELECT_BUTTON,
//                disabled: true,
//                click: dialogOptions.onOkButtonClick
//            }),
            $("<button />", {
                'class': "nc_admin_metro_button_cancel nc-btn nc--red nc--bordered nc--right",
                html: lang.DIALOG_CANCEL_BUTTON,
        ...
        var initTree = function() {
            var dialog = $('#' + dialogId),
                treeUrl = dialogOptions.treeSourceUrl.replace("%siteId%", dialog.data('siteId')),
                tree = new window.top.dynamicTree(treeId, treeId, treeUrl);

            dialog.data('tree', tree);
            dialog.find(".list-column").html('');

            tree.actions = dialogOptions.treeActions;
        ...
        // load dynamicTree script and initialize the tree when the dialog is opened
        dialog.data({
            onShow: function(modal) {
                $(window.top).resize(); // trigger dialog size change

                // reload contents only when 'doNotReload' flag of the dialog is false
                // 'modal' is a 'jQuery.modal' object, the dialog div is 'modal.data'
                if (modal.data.data("doNotReload") == true) { return; }

                // load the "dynamic tree" script and initialize it
                if (!window.top.dynamicTree) {
                    var cacheValue = $.ajaxSetup()['cache'];
                    $.ajaxSetup({ cache: true });
                    $.getScript(Class.urls.DynamicTreeScript, initTree);
                    $.ajaxSetup({ cache: cacheValue });
                }
                else {
                    initTree();
                }
     ...
     * dialog.data = {
     *      siteId: id,
     *      targetInput: hidden input,
     *      targetCaption: span for the caption
     *      tree: dynamicTree,
     *      doNotReload: if true, do not re-initialize the dialog contents (tree and list columns)
     *      onShow: function to invoke when dialog is showed (modal.onShow handler)
     * }
     */
    Instance.createItemSelectionDialog = function() {
        var $ = window.top.$nc,
            dialogId = Class.dialogIds['itemSelection'],
            options = {
                dialogId: dialogId,
                headerText: lang.ITEM_SELECTION,
                treeSourceUrl: Class.urls.ObjectSubdivisionTree,
                treeActions: {}
//                onOkButtonClick: null
            };

        // handler for the click on the item in the list (right column)
        var selectItem = function() {
            ...
            dialog.data("targetInput").val(el.data('object'));
            dialog.data("targetCaption").html(el.html());
            $.modal.close();
        };

        // handler for the tree node click
        options.treeActions.selectNode = function(id) {
            var dialog = $('#' + dialogId),
                listColumn = dialog.find('.list-column').html('');

            $.get(Class.urls.ObjectList,
                  { subdivision_id: id },
                  function(response) {
                      // a closure here might fail, so  ID + class is used to select the target div
                      listColumn.html(response)
                                .find(".item").click(selectItem);
                  });
            dialog.data('tree').selectNode('sub-'+id);
            return false;
        };

        return this.createTreeDialog(options);
    };
    ...
    Instance.showItemSelectionDialog = function(inputField, captionContainer) {
        var $ = window.top.$nc,
            dialog = $('#' + Class.dialogIds.itemSelection);

        if (dialog.length == 0) { // create new dialog
            dialog = this.createItemSelectionDialog();
        }

        dialog.data({
            doNotReload: this.siteId && (this.siteId == dialog.data('siteId')),
            siteId: this.siteId,
            targetInput: inputField,
            targetCaption: captionContainer
        });

        $.modal(dialog, { onShow: dialog.data('onShow'), persist: true });
    };
     ...
     *
     */
    Instance.createUserSelectionDialog = function() {
        var $ = window.top.$nc,
            dialogId = Class.dialogIds['userSelection'],
            options = {
                dialogId: dialogId,
                headerText: lang.USER_SELECTION,
                treeSourceUrl: Class.urls.UserGroupTree,
                treeActions: {}
//                onOkButtonClick: null
            };
        ...
        var selectItem = function() {
            ...
            dialog.data("targetInput").val(el.data('user-id'));
            dialog.data("targetCaption").html(el.html());
            $.modal.close();
        };
        ...
        options.treeActions.selectNode = function(id) {
            ...
            dialog.data('tree').selectNode('usergroup-'+id);
            return false;
        };
    ...
    Instance.showUserSelectionDialog = function(inputField, captionContainer) {
        var $ = window.top.$nc,
            dialog = $('#' + Class.dialogIds.userSelection),
            dialogIsInitialized = dialog.length > 0;

        if (!dialogIsInitialized) { // create new dialog
            dialog = this.createUserSelectionDialog();
        }

        dialog.data({
            doNotReload: dialogIsInitialized,
            targetInput: inputField,
            targetCaption: captionContainer
        });
    ...
    Instance.fillSelectsWithOptions = function(selector, data) {
        ...
        if (selects.length > 0) {
            var options = ["<option></option>"], optionValue;
            for (optionValue in data) {
                options.push('<option value="' + optionValue + '">' + data[optionValue] + '</option>');
            }
    ...
    Instance.onSelectItemPropertyListReady = function(data, url, callbackParams) {
        Class.itemProperties = data; // store info about fields in nc_netshop_condition_editor.itemProperties to reuse it later
    ...
    Instance.generatePropertyFieldConfiguration = function(property) {
        ...
        switch (property.type) {
            ...
            case 'multiselect':
                ...
                break;
            case 'boolean':
                fields.push(lang.EQUALS, { name: 'value', type: 'SimpleSelect', values: {0: lang.TRUE, 1: lang.FALSE }});
                break;
            case 'datetime':
    ...
    Instance.getItemPropertyData = function(propertyId) {
        var properties = Class.itemProperties;
    ...
    Instance.onSelectUserPropertyListReady = function(data, url, callbackParams) {
        Class.userProperties = data; // store info about fields in nc_netshop_condition_editor.userProperties to reuse it later
    ...
    Instance.onUserPropertySelect = function(event) {
        var select = $(event.target),
            property = Class.userProperties[select.val()];
    ...
    Instance.onSelectOrderPropertyListReady = function(data, url, callbackParams) {
        Class.orderProperties = data; // store info about fields in nc_netshop_condition_editor.orderProperties to reuse it later
    ...
    Instance.onOrderPropertySelect = function(event) {
        var select = $(event.target),
            property = Class.orderProperties[select.val()];
