...
$row->select('data[amount_type]', array(
            nc_netshop_promotion_discount_item::TYPE_ABSOLUTE => $currency_name,
            nc_netshop_promotion_discount_item::TYPE_RELATIVE => '%'
...
if ($discount_type == 'delivery') {
    $condition_groups_to_exclude = array('GROUP_GOODS');
}
...
(function() {
    var condition_editor = new nc_netshop_condition_editor(
        '#nc_netshop_condition_editor',
        'data[condition]',
        <?=$condition_json ?>,
        <?=$catalogue_id ?>,
        [],
        <?=nc_array_json($condition_groups_to_exclude) ?>
    );
