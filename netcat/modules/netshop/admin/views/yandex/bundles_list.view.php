...
foreach ($bundles as $row) {
    ...
    $post_actions_params = array('controller' => $controller_name, 'bundle_id' => $bundle_id);

    $tr->id->text($bundle_id);
    $tr->name->text(
        "<a href='$edit_link' target='_top' class='nc-netshop-list-item-title'>" . $bundle->get('name') . "</a>"
    );
    $tr->export_url->text('http://'.$domain.'/netcat/modules/netshop/export/yandex/bundle'.$bundle_id.'.xml');
