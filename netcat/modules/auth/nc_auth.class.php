...
class nc_auth {
    ...
    protected function __construct() {
        ...
        $this->auth_view = new nc_module_view();
...
NETCAT_ADD_FORM;
    ...
    public function need_captcha() {
        ...
        if (!$max_n) return false;
        $need_captcha = 0;
        if ($_SESSION['ncAttemptAuth'] >= $max_n) return true;
        if ($_COOKIE['ncAttemptAuth'] >= $max_n) return true;
    ...
    public function get_confirm_mail($user_id, $password = null) {
        ...
        $subject = $this->core->get_settings('mail_confirm_subject', 'auth');
        $body = $this->core->get_settings('mail_confirm_body', 'auth');
        $is_hrml = $this->core->get_settings('mail_confirm_is_html', 'auth');
        ...
        $macro = array('SITE_NAME' => $this->core->catalogue->get_current('Catalogue_Name'),
                ...
                'PASSWORD' => $password ? $password : $this->core->input->fetch_get_post('Password1'),
                'CONFIRM_LINK' => "http://".$_SERVER['HTTP_HOST'].$SUB_FOLDER.$HTTP_ROOT_PATH."modules/auth/confirm.php?id=".$userinfo['User_ID']."&code=".$userinfo['RegistrationCode']);
    ...
    public function get_recovery_mail($user_id, $confirm_code) {
        ...
        $confirm_link = "http://".$HTTP_HOST.$SUB_FOLDER.$HTTP_ROOT_PATH."modules/auth/password_recovery.php?sub=".$sub."&uid=".$user_id."&ucc=".$confirm_code;

        $userinfo = $this->db->get_row("SELECT * FROM `User` WHERE `User_ID` = '".intval($user_id)."' ", ARRAY_A);
        $subject = $this->core->get_settings('mail_recovery_subject', 'auth');
        $body = $this->core->get_settings('mail_recovery_body', 'auth');
        $is_html = $this->core->get_settings('mail_recovery_is_html', 'auth');
        ...
        return array('subject' => $subject, 'body' => $body, 'html' => $is_html);
    }

    public function get_notify_admin_mail($user_id) {
        $userinfo = $this->db->get_row("SELECT * FROM `User` WHERE `User_ID` = '".intval($user_id)."' ", ARRAY_A);
        $subject = $this->core->get_settings('mail_notify_admin_subject', 'auth');
        $body = $this->core->get_settings('mail_notify_admin_body', 'auth');
        $is_hrml = $this->core->get_settings('mail_notify_admin_is_html', 'auth');

        $macro = array('SITE_NAME' => $this->core->catalogue->get_current('Catalogue_Name'),
                'SITE_URL' => $_SERVER['HTTP_HOST'],
                'USER_LOGIN' => $userinfo[$this->core->AUTHORIZE_BY],
                'USER_NAME' => $userinfo['ForumName'],
                'PASSWORD' => $this->core->input->fetch_get_post('Password1'),
                'CONFIRM_LINK' => "http://".$_SERVER['HTTP_HOST'].$this->core->SUB_FOLDER.$this->core->HTTP_ROOT_PATH."modules/auth/confirm.php?id=".$userinfo['User_ID']."&code=".$userinfo['RegistrationCode']);
