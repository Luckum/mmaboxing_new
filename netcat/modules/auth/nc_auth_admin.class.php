...
class nc_auth_admin {
    ...
    public function classic_save() {
        ...
        // настройки
        foreach ($params as $v) {
            $this->core->set_settings($v, intval($this->core->input->fetch_get_post($v)), 'auth', $catalogue_id);
        }
        ...
        $this->core->set_settings('field_custom', join(',', $f), 'auth', $catalogue_id);
        ...
        $this->core->set_settings('group', join(',', $f), 'auth', $catalogue_id);
    }
    ...
    public function templates_show() {
        ...
        $auth_editor = new nc_module_editor(true);
    ...
    public function ex_show() {
	        ...
	        if ($v == 'oauth') {
		        $html = "<div id='oauth_providers'>
		                    ...
		                    <div class='oauth_header'>
						        <div class='img'><img src='/netcat/admin/skins/default/img/oauth.png'></div>
      ...
      nc_mf_oauth = new nc_mapping_fields({ " . join(',', $js_field_user) . " }, { 'uid' : 'ID', 'name': '" . NETCAT_MODULE_AUTH_FIRST_NAME . "', 'nick': '" . NETCAT_MODULE_AUTH_NICKNAME . "', 'photo' : '" . NETCAT_MODULE_AUTH_PHOTO . "'}, 'oauth_mapping', 'nc_mf_oauth', '" . NETCAT_MODULE_AUTH_ADMIN_EX_DATA_OAUTH . "' );
