...
class nc_backup_subdivision extends nc_backup_base {
    ...
    protected function event_before_insert_subdivision($row) {
        if ($row['Parent_Sub_ID']) {
            $row['Parent_Sub_ID'] = $this->dumper->get_dict('Subdivision_ID', $row['Parent_Sub_ID'], $row['Parent_Sub_ID']);
        }
    ...
    protected function event_before_insert_sub_class($row) {
        $row['Subdivision_ID'] = $this->dumper->get_dict('Subdivision_ID', $row['Subdivision_ID'], $row['Subdivision_ID']);
        $row['Class_ID'] = $this->dumper->get_dict('Class_ID', $row['Class_ID'], $row['Class_ID']);
// idebug($row);
// exit;
        return $row;
    }
