...
class nc_backup_widget_class extends nc_backup_base {
    ...
    protected function event_after_insert_widget_class($data, $insert_id) {
        $update = array('File_Path' => "/{$insert_id}/");
        $this->widget_class_table->where_id($insert_id)->update($update);
    }

    //--------------------------------------------------------------------------

    protected function event_before_copy_file($path, $file) {
        return $path . $this->dumper->get_dict('Widget_Class_ID', $file);
    }
