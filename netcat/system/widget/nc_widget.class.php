...
class nc_Widget extends nc_System {
    ...
    public function generate($id, $fields = '', $static = 0, $widget_id = 0) {
        ...
        $res = (array) $db->get_results("SELECT `Field_Name`, `DefaultState` FROM `Field` WHERE `Widget_Class_ID`='" . $params['Widget_Class_ID'] . "'");
        ...
        if ($params['IsStatic']) {
            ...
            if ( !empty($fields) ) foreach ($fields as $f => $val) {
                $new_fields .= "\$f_" . $f . "= '" . $val . "';";
            }
            if ($new_fields) eval($new_fields);
        } else {
            foreach ($res as $r) {
                ${'f_' . $r->Field_Name} = $r->DefaultState;
            }
            if ( !empty($fields) ) extract($fields, EXTR_PREFIX_ALL, "f");
        }
    ...
    public function show($id, $for_macrofunc = 0) {
        ...
        if (!$result['isStatic'] || (intval($result['Update'] != 0)) && (time() - intval($result['Update']) * 60 >= strtotime($result['LastUpdated']))) {
            ...
            if ($result['Keyword'] == 'nc_aggregator') {
                $ret_val = $this->generate_aggregator($result['Widget_ID']);
            } else {
