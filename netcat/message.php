            ...
            if (!(
                ...
                if ($multiple_changes) {
                    ...
                    if ($user_table_mode) {
                        ...
                        $KeywordStr = !$KeywordDefined ? "`Keyword` = '" . nc_check_keyword_name($message, $f_Keyword, $classID) . "', " : "";
                ...
                for ($i = 0; $i < count($tmpFile); $i++) {
                    ...
                    @rename($FILES_FOLDER . 'preview_' . $tmpFile[$i], $FILES_FOLDER . $File_Path[$i] . 'preview_' . $tmpNewFile[$i]);
                    @chmod($FILES_FOLDER . $File_Path[$i] . $tmpNewFile[$i], $FILECHMOD);
                    @chmod($FILES_FOLDER . $File_Path[$i] . 'preview_' . $tmpNewFile[$i], $FILECHMOD);
                }
