...
if ($perm->isAccessSiteMap() || $perm->isGuest()) {
	...
	if ( !empty($result) ) {
		foreach ($result as $row) {
			...
			if (isset($sub_classes[$row->Subdivision_ID])) continue;
...
<div class="nc-widget-scrolled">
	<div class="nc-position-t nc-bg-dark" style="background:rgba(0,0,0,.05)">
		...
		<? if ($perm->isAccess(NC_PERM_FAVORITE)): ?>
			<a class='nc--right' href='<?=$SUB_FOLDER . $HTTP_ROOT_PATH ?>admin/subdivision/favorites.php?phase=1' onclick='return nc.ui.dashboard.fullscreen(this)'><i class="nc-icon nc--settings nc--white"></i> <?=SECTION_INDEX_FAVORITE_SETTINGS ?></a>
		<? endif ?>
	</div>
<? if ( empty($catalogues) ): ?>
			...
			<td class="nc-text-right">
				<? if ($cat['domain_error']['text']): ?>
					<? if ($cat['domain_error']['link']): ?>
						<a href="<?=$cat['domain_error']['link'] ?>" class='nc-label nc--light'><?=$cat['domain_error']['text'] ?></a>
					<? else: ?>
						<span class='nc-label nc--lighten'><?=$cat['domain_error']['text'] ?></span>
					<? endif ?>
				<? endif ?>
				<a class='nc-label nc--lighten' href='subdivision/full.php?CatalogueID=<?=$cat_id ?>'><?=NETCAT_TREE_SITEMAP ?></a>
			</td>
		</tr>
		...
		<? foreach ($favorites[$cat_id] as $sub_id => $sub): ?>
			...
			<tr>
				<td colspan='2'>
					...
					<a href='<?=$link ?>' title='<?=$sub['Subdivision_Name'] ?>' <?(!$sub['SubChecked'] ? " class='nc--disabled'" : "") ?>>
						<?=$sub['Subdivision_Name'] ?>
					</a>

					<div class="nc--right">
						<? if(isset($subclass_link[$sub_id])): ?>
							<a href='<?=$subclass_link[$sub_id] ?>'><i class='nc-icon nc--edit nc--white nc--hovered' title='<?=CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_EDIT ?>'></i></a>
						<? endif ?>
						<a href='<?=nc_subdivision_preview_link($sub) ?>' target='_blank'><i class='nc-icon nc--arrow-right nc--white nc--hovered' title='<?=CONTROL_CONTENT_SUBDIVISION_FUNCS_TOVIEW ?>'></i></a>
					</div>
				</td>
			</tr>
		<? endforeach ?>
