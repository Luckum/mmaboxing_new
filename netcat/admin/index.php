...
<head>
    ...
    $js_files[] = $ADMIN_PATH . 'js/typofilter.js';
    ...
    <script type='text/javascript' src='<?= $ADMIN_PATH ?>js/typofilter.js?<?= $LAST_LOCAL_PATCH ?>'></script>*/
    ?>
    <?
...
if ($perm->isUserMenuShow()) {
    ...
    if ( $perm->isAccess(NC_PERM_ITEM_GROUP, 0, 0, 0) ) {
        $navbar->menu->users
            ->add_btn('#usergroup.list', SECTION_CONTROL_USER_GROUP)->icon('user-group')
            ->add_btn('#user.mail', SECTION_INDEX_USER_USER_MAIL)->icon('mod-subscriber')->divider();
    }
}
...
if ($perm->isSupervisor() || $perm->isGuest()) {
    ...
    $navbar->menu->tools
        ...
        ->add_btn('#redirect.settings', TOOLS_REDIRECT)->icon('redirect')
        ->add_divider();
