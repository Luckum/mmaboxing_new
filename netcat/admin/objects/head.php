        ...
        <script>
            ...
            function loadSubClasses(subId, ccId, classId, messageId) {
                document.getElementById('subViewIframe').contentWindow.location =
                    '<?=$ADMIN_PATH
?>objects/select_subclass_list.php?sub='+subId+'&cc='+ccId+'&classID='+classId+'&message='+messageId;
            }

            function copyItem(subId, ccId, classId, messageId) {
                window.location = '<?=$ADMIN_PATH
?>objects/save.php?<?=$nc_core->token->get_url()
?>&sub='+subId+'&cc='+ccId+'&classID='+classId+'&message='+messageId;
            }
        </script>
    </head>
