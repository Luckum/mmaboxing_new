...
/* $Id: admin.inc.php 8523 2012-12-12 09:29:45Z ewind $ */
...
function nc_array_json($array) {
    ...
    sort($array_keys);
    $array_is_numeric = ($array_keys[0] == "0" ? true : false);

    $ret_values = array();
    foreach ($array as $k => $v) {
        if (is_object($v)) {
            continue;
        } // ВНУТРЕННИЕ ОБЪЕКТЫ, тонкое место
        elseif (is_array($v)) {
            $v = nc_array_json($v);
        } elseif (is_bool($v)) {
            $v = $v ? 'true' : 'false';
    ...
    function init($sub_id, $cc_id = 0, $message_id = 0) {
        ...
        if ($this->sub->is_subdivision_admin()) {
            $this->tabs[] = array('id' => 'settings',
                ...
                'location' => "subdivision.edit({$this->sub->subdivision_id})"
            );
        }
...
function nc_postprocess_admin_page($str) {
    if (!nc_strlen($str)) { return ""; }

    global $inside_admin;
    if (!$inside_admin) { return $str; }

    $script = "\n<script>\ntop.nc_add_inside_admin_parameter && top.nc_add_inside_admin_parameter();\n</script>\n";
...
//  if ($dest_subclass['Sub_Class_ID'] == $message['Sub_Class_ID']) { return true; } // Проверка на копирование объекта внутри одного $cc
    ...
    if (!empty($file_fields)) {
        ...
        foreach ($file_fields as $field) {
            ...
            if (!$message[$field['Field_Name']]) continue;
            ...
            //исходный файл
            $src_file_path = nc_file_path($class_id, $message_id, $field['Field_ID']); // его путь
            $value_array = explode(':', $message[$field['Field_Name']]);
            $src_file_name = $value_array[0]; // оригинальное имя
            $src_file_type = $value_array[1]; // тип
            $src_file_size = $value_array[2]; // размер
            $ext = substr($src_file_name, strrpos($src_file_name, ".")); // расширение
            // в зависимости от ФС менятеся папка и имя на диске + значени в БД
            switch ($fs) {
                case NC_FS_SIMPLE:
                    $put_file_name = $field['Field_ID'] . "_" . $new_message_id . $ext; // имя файла: IDполя_IDобъекта.расширение
                    $FilePath = ''; // в папку netcat_files
                    $fieldValue = $src_file_name . ":" . $src_file_type . ":" . $src_file_size; // значение в базу
                    break;
                case NC_FS_ORIGINAL:
                    ...
                    $fieldValue = $src_file_name . ":" . $src_file_type . ":" . $src_file_size . ":" . $FilePath . $put_file_name; // значение в базу
                    break;
    ...
    foreach ((array)$multifile_fields as $field) {
        ...
        foreach ((array)$files as $file) {
            ...
            $sql = "INSERT INTO `Multifield` (`Field_ID`, `Message_ID`, `Priority`, `Name`, `Size`, `Path`, `Preview`) VALUES " .
                "({$field_id}, {$new_message_id}, {$priority}, '{$name}', {$size}, '{$path}', '{$preview}')";
            $db->query($sql);
        }
    }
...
function nc_admin_textarea($disc, $name, $value, $is_resizeble = 0, $fck = 0, $style = '') {
    ...
    if ($is_resizeble) {
        $ret .= "<div class='resize_block'><a class='textarea_shrink' href='#" . $name . "' > &#x25BC; </a>
...
class nc_admin_fieldset {
    ...
    public function __construct($name = '', $mode = 'on') {
    ...
    public function result() {
        ...
        self::$style = '';
        return $style .
        "<div class='nc_admin_fieldset'" . ($this->show ? '' : " style='display: none;'") . ">
                    " . ($this->name ? "
                    <div class='nc_admin_fieldset_head'>
                        {$this->name}
                    </div>" : "") . "

                    <div class='nc_admin_fieldset_body'>
                        {$this->text}
                    </div>
                </div>";
    }
