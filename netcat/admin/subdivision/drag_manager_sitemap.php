...
/* $Id: drag_manager_sitemap.php 8580 2012-12-28 12:45:02Z ewind $ */
/**
 * Обрабатывает D&D:
...
if (!$dragged_id || !$target_id) die("0 /* Wrong parameters */");
...
if ($dragged_type == 'sub' && ($target_type == 'sub' || $target_type == 'site')) {
    ...
    if (!$dragged || !$target) die("0 /* Wrong IDs */");
    ...
    if ($dragged['Parent_Sub_ID'] == $parent_sub) {
        ...
        $subdivisions_arr = $db->get_col("SELECT `Subdivision_ID` FROM `Subdivision`
      WHERE `Parent_Sub_ID` = '".$parent_sub."' AND `Priority` >= '".$priority."'");
    ...
    // execute core action
    $nc_core->event->execute("updateSubdivisionPrep", $dragged['Catalogue_ID'], $dragged['Subdivision_ID']);
    ...
    // execute core action
    $nc_core->event->execute("updateSubdivision", $dragged['Catalogue_ID'], $dragged['Subdivision_ID']);

    if (count($chil_sub_id)) {
        foreach ($chil_sub_id as $v) {
            // execute core action
            $nc_core->event->execute("updateSubdivisionPrep", $target['Catalogue_ID'], $v);

            $db->query("UPDATE `Subdivision` SET `Catalogue_ID` = '".$target['Catalogue_ID']."' WHERE `Subdivision_ID` = '".$v."'");
            // execute core action
            $nc_core->event->execute("updateSubdivision", $target['Catalogue_ID'], $v);
            $subclasses = $db->get_col("SELECT `Sub_Class_ID` FROM `Sub_Class` WHERE `Subdivision_ID` = '".$v."'");
            if (!empty($subclasses)) {
                // execute core action
                $nc_core->event->execute("updateSubClassPrep", $target['Catalogue_ID'], $v, $subclasses);
...
if ($dragged_type == 'site' && $target_type == 'site') {
    ...
    if ($perm->isCatalogueAdmin($dragged_id) && $perm->isCatalogueAdmin($target_id)) {
        ...
        $nc_core->event->execute("updateCataloguePrep", $catalogues);
        ...
        $nc_core->event->execute("updateCatalogue", $catalogues);
    }
