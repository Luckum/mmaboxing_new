...
function nc_subdivision_form_edit_save() {
    ...
    $EnglishName = nc_check_english_name($sub_id, $EnglishName, 1);
 ...
 * @return string
 */
function nc_check_english_name($obj_id=0, $string = "", $type = 1) {
  global $db;
  switch ($type) {
    case 1:
      $table = "Subdivision";
  ...
  $sql = "SELECT COUNT(*) as matches FROM ".$table." WHERE EnglishName = '".$db->escape($string)."' AND ".$table_id." <> ".intval($obj_id)." ";
