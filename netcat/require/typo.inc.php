...
function nc_typo($text, $settings = 'none') {
    ...
    // Т._к., т._д., т._е.
    $text = preg_replace('/([а-яА-Я])\.\s*([а-я])\.(\s*)(,?)(\s*)(\S+|$)/u', '\1.&nbsp;\2.\3\4 \6', $text);
    ...
    // А. Б. Шеин
    $text = preg_replace('/(\W)([А-ЯA-Z]\.)\s*([А-ЯA-Z]\.)\s*([А-ЯA-Z][а-яa-z]+)/u', '\1\2&nbsp;\3&nbsp;\4', $text);
    // А. Шеин
    $text = preg_replace('/(\W)([А-ЯA-Z]\.)\s*([А-ЯA-Z][а-яa-z]+)/u', '\1\2&nbsp;\3', $text);
    // Шеин А. Б.
    $text = preg_replace('/(\W)([А-ЯA-Z][а-яa-z]+)\s+([А-ЯA-Z]\.)(\s*([А-ЯA-Z]\.))?/u', '\1\2&nbsp;\3&nbsp;\5', $text);
    // Потому_что
