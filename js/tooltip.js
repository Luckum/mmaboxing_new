...
// Скрипт взят с сайта http://www.focusoff.net
//
// Добавляем события для нужных нам объектов:
...
// <span onmouseout="toolTip()" onmouseover="toolTip('Привет я скрипт ToolTip','red','#FFFFFF')">Наведите курсор мыши на эту надпись</span>

/*
Разместите этот код внутри тега<body>, лучше сразу после него 
...
function toolTip(msg,fg,bg)
{
  ...
  { // Если функция toolTip пустая - скрываем содержимое слоя
    if( ns4 ) toolTipSTYLE.visibility="hidden";
    else toolTipSTYLE.display="none";
    vmsg=''; 
    return;
   }
  // Если непустая - Отображаем слой
  if( vmsg!='' ) return;
  vmsg=msg;
  if( !fg ) fg="#000000"; // Цвет текста по умолчанию
  if( !bg ) bg="#FFFFFF"; // Цвет фона по умолчанию
  var content=
      '<table border="solid 1px" cellspacing="0" cellpadding="1" bgcolor="'+fg+'">'
      +'<td>' // Шаблон выводимого окна подсказки
...
function toolClick()
{
  ...
  fg="#000000"; // Цвет текста по умолчанию
  bg="#FFFFFF"; // Цвет фона по умолчанию
