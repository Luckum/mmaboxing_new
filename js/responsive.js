...
(function($){
	$(document).ready(function(){
		// $('#page_comment_block').text('Ваш комментарий');
		$('#nc_commentsSubmitButton').text('Оставить ваш коментарий');
        $('#weight_cat').styler();
		...
		if ($(window).width() <= 1281 && $('#adp_menu').length == '0') {
			...
			$('.head').before($('.menu_count'));	
			$('#adp_menu').prepend('<div class="top_adp_menu"></div>');
			$('.top_adp_menu').prepend($('.send_article'));
			$('.top_adp_menu').prepend($('.user'));
			$('#adp_menu').prepend('<div class="slide_container"></div>');
			$('.slide_container').prepend($('.top_adp_menu, .menu_count'));
			$('.slide_container').hide();
			$('.search').after($('.logo'));
			$('.head').append('<div class="humburger"><span class="l1"></span><span class="l2"></span><span class="l3"></span></div>');
			$('.logo').after($('.humburger'));
			document.getElementsByClassName('humburger')[0].onclick = function() {
        ...
        if ($(window).width() <= 1281) {
            ...
            $('.bckClickertop').hide();
            $('.logo img').attr('src', '/images/logo-on-dark.png');
            $('#v-iframe').attr('width', $(window).width() - 8);
            $('.to-slide').hide();
            $('.not-slide').show();
            createBanner();
        }
...
{
    $("body").append("<div id='banner-m' class='banner-m'></div");
    $("#banner-m").append("<img class='banner-m-close' src='/images/close_banner.png'>");
    $.ajax({
        url: "/netcat/modules/default/ajax.php?isNaked=1",
        type: "POST",
        data: {action: 'get_banner'},
        complete: function(response) {
            var data = $.parseJSON(response.responseText);
            if (data.type == 1) {
                $("#banner-m").append("<a target='_blank' href='" + data.link + "'><img src='" + data. src + "'></a>");
            } else {
                $("#banner-m").append(data.src);
            }
        }
    });
}