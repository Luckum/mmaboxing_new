var share_fgt = {
        ...
        if(apiUrl && pageUrl){
            if(type == "twitter"){
                var tw_cnt = $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    data: {action: 'get_tw_cnt_fgt', page: pageUrl}
                }).responseText;
                
                if (tw_cnt > 0) {
                    share_fgt.setCountLikes(box, tw_cnt);
                } else {
                    share_fgt.setCountLikes(box, '0');
                }
            };
            if(type == "vk"){
                var vk_cnt = $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    data: {action: 'get_vk_cnt_fgt', page: pageUrl}
                }).responseText;
                
                if (vk_cnt > 0) {
                    share_fgt.setCountLikes(box, vk_cnt);
                } else {
                    share_fgt.setCountLikes(box, '0');
                }
            };
            if(type == "facebook"){
                var fb_cnt = $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    data: {action: 'get_fb_cnt_fgt', page: pageUrl}
                }).responseText;
                
                if (fb_cnt > 0) {
                    share_fgt.setCountLikes(box, fb_cnt);
                } else {
                    share_fgt.setCountLikes(box, '0');
                }
            };
    ...
    twitter: function($this){
        ...
        if(data){
            ...
            share_fgt.popup($this, url, data.id, 't');
        };
        return false;
    },
    vk: function($this){
        ...
        if(data){
            var url  = 'http://vkontakte.ru/share.php?';
                ...
                url += '&noparse=true';
            share_fgt.popup($this, url, data.id, 'v');
        };
        return false;
    },
    facebook: function($this){
        ...
        if(data){
            ...
            share_fgt.popup($this, url, data.id, 'f');
        };
        return false;
    },
    data: function($this){
    ...
    popup: function(box, url, id, type){
        ...
        if(!countBox.length){
            share_fgt.setCountLikes(box, 1);
        }else{
            countBox.text(parseInt(countBox.text()) + 1);
        };
        if (type == 't') {
            $.post('/netcat/modules/default/ajax.php?isNaked=1', {action: 'set_tw_cnt_fgt', page: id}, function (data){});
        }
        if (type == 'f') {
            $.post('/netcat/modules/default/ajax.php?isNaked=1', {action: 'set_fb_cnt_fgt', page: id}, function (data){});
        }
        if (type == 'v') {
            $.post('/netcat/modules/default/ajax.php?isNaked=1', {action: 'set_vk_cnt_fgt', page: id}, function (data){});
        }
